...
def tune(tuner, dataset, num_epochs, batch_size, prefix_len, lr):
    if dataset == 'small':
        files = {"train": DATA_PATH + "small_data.txt",
                 "val": None}
    elif dataset == 'medium':
        files = {"train": DATA_PATH + "medium_data.txt",
                 "val": DATA_PATH + "medium_data_val.txt"}
    elif dataset == 'full':
    ...
    val_dataloader = dataloaders["val"] if "val" in dataloaders.keys() else None

    optimizer = optim.AdamW(model.parameters(), lr=lr)
    train_losses, val_losses = train(model, optimizer, train_dataloader, val_dataloader, num_epochs)
...
def train(model, optimizer, train_dataloader, val_dataloader=None, epochs=10):
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=100, num_training_steps = epochs*len(train_dataloader))
...
if __name__ == "__main__":
    ...
    # parse yaml
    hyperparameter_file = Path(__file__).parent / "configs" / "hyperparameters.yaml"
